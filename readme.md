...
当然在正式项目中应该用专门原构建工具，避免手写编译命令。如果按下节构建方式安装
...
可选使用 `make test` 运行单元测试，用这个单元测试本身测试其主要功能。
可用 `make test -n` 查看如何手动运行单元测试程序。

### 用 CMake 构建

也提供了 `cmake` 构建系统，如下标准的构建与安装命令：
...
其中第一种的三参数形式与第二种的显式比较 `true` 等效。在只有双参数浮点数，采用
...
* `COUT_EXCEPTION(statement, message)` 断言执行语句 `statement` 会抛出标准异常
  ...
  认为是字符串 `const char*` 类型，空串 `""` 能匹配任意导演消息，但如果是
  `nullptr` 则表明 `statement` 不抛异常。
...
这两个文件是否相同的判据。在被测程序有处理文件相关功能时，可用该宏简化测试。

### DESC 描叙宏

宏 `DESC(msg, ...)`，类似 `printf` 调用，但自动加换行符。
主要为了改善输出信息的可读性，在有需要时起到能输出到结果的注释作用。
...
* 如果参数是 `*.cpp` 文件名及可选附加 `:行号`，则只执行在该文件及某行号之后定
  义的测试用例。
* 查找包含给定参数子串的用例名。
...
* 以一个短横 `-` 引导的参数，表示不执行匹配的用例，相应的可选的 `+` 前缀表示执
  行匹配的用例。
* 仅一个 `+` 没有词根时，等效于 `--all` 参数，加入所以测试用例，包括 `DEF_TOOL`
  定义的用例。
...
试用例，以及带上全名单测某一个或几个测试用例。良好的单元测试用例设计，应该使每
...
本 `couttast` 库只处理位置参数及可识别的选项参数，其他参数在运行时可通过
...
如果在命令行与 ini 配置文件中存在同名选项参数，以命令行输入的为准。配置文件的
位置参数接在命令行输入的位置参数之后。
...
`main()` 函数，覆盖默认提示的 `main()` 函数。比如在调用 `tast::main()` 之前先
做些其他预处理工作。
...
单元测试运行，这就相当于包含许多子命令模式的命令行程序，每个单元测试用例相当于
一个子命令。
...
* 在扩展库 `couttast.h` 中还定义了 `tast::GetOption()` 函数用于获取命令行选项。
...
在某些情况下，可能需要测试或验证私有方法。在 C++ 可能有些黑魔法可以实现访问私
有方法，但这里提供一种简单朴素，易懂易操作的方法。就是基于宏定义，但也不能直接
给编译器传全局宏定义 `g++ -Dprivate=public` ，因为这会导致一些标准库编不过。
所以只宜对局部被测类施以重定义访问权限。

假设开发一个类库，有源文件 `foo.cpp`，头文件 `foo.h` ，为它写单元测试的源文件
叫 `test-foo.cpp` 。那么只要在 `foo.h` 文件中将被测类定义中的 `private` 改为
`private__` 多加两个下划线，并在开头包含 `private-access.hpp` ，然后在测试源文
...
极尽优雅了，但自己开发时对实现的正确性不够自信，想要写单元测试又该如何呢？
...
不过此法要注意的是测试中不宜输出指针地址，因为不同运行时内存地址不可能做到保持一致。
...
其他用 `DEF_TAST` 定义的功能测试用例按逻辑分开。默认地不执行，只响应显式参数执
行。
...
* 为 `COUT` 宏增加一系列 `COUT_BIT_*` 常量控制各类信息的输出，由此 Print 方法
  需要额外的标志参数。
